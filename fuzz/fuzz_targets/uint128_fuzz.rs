...
fn do_test(data: &[u8]) {
    macro_rules! read_ints {
        ($start: expr) => { {
            ...
            // Note BE:
            let uint128 = tapyrus::util::uint::Uint128::from(&[native as u64, (native >> 8*8) as u64][..]);

            // Checking two conversion methods against each other
            let mut slice = [0u8; 16];
            slice.copy_from_slice(&data[$start..$start + 16]);
            assert_eq!(uint128, tapyrus::util::uint::Uint128::from_be_bytes(slice));
    ...
    assert_eq!(a_native as u64, tapyrus::util::uint::Uint128::from_u64(a_native as u64).unwrap().low_u64());
