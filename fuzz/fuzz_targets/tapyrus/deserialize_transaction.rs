...
fn do_test(data: &[u8]) {
    let tx_result: Result<tapyrus::blockdata::transaction::Transaction, _> =
        tapyrus::consensus::encode::deserialize(data);
    match tx_result {
        Err(_) => {}
        Ok(mut tx) => {
            ...
            for input in &mut tx.input {
                input.witness = tapyrus::blockdata::witness::Witness::default();
            }
