...
impl Psbt {
    ...
    /// 1000 taps/vByte. 25k taps/vByte is obviously a mistake at this point.
    ///
    /// [`extract_tx`]: Psbt::extract_tx
    pub const DEFAULT_MAX_FEE_RATE: FeeRate = FeeRate::from_tap_per_vb_unchecked(25_000);
    ...
    fn internal_extract_tx_with_fee_rate_limit(
        ...
        let fee_rate =
            FeeRate::from_tap_per_kwu(fee.to_tap().saturating_mul(1000) / tx.weight().to_wu());
    ...
    pub fn fee(&self) -> Result<Amount, Error> {
        ...
        for utxo in self.iter_funding_utxos() {
            inputs = inputs.checked_add(utxo?.value.to_tap()).ok_or(Error::FeeOverflow)?;
        }
        let mut outputs: u64 = 0;
        for out in &self.unsigned_tx.output {
            outputs = outputs.checked_add(out.value.to_tap()).ok_or(Error::FeeOverflow)?;
        }
        inputs.checked_sub(outputs).map(Amount::from_tap).ok_or(Error::NegativeFee)
    }
}
...
mod tests {
    ...
    fn psbt_high_fee_checks() {
        ...
        assert_eq!(
            ...
            Err(FeeRate::from_tap_per_kwu(15060240960843))
        );
        assert_eq!(
        ...
        assert_eq!(
            psbt.clone()
                .extract_tx_with_fee_rate_limit(FeeRate::from_tap_per_kwu(15060240960842))
                .map_err(|e| match e {
                    ExtractTxError::AbsurdFeeRate { fee_rate, .. } => fee_rate,
                    _ => panic!(""),
                }),
        ...
        assert!(psbt
            .extract_tx_with_fee_rate_limit(FeeRate::from_tap_per_kwu(15060240960843))
            .is_ok());

        // Testing that extract_tx will error at 25k tap/vbyte (6250000 tap/kwu)
        assert_eq!(
            psbt_with_values(2076001, 1000).extract_tx().map_err(|e| match e {
                ExtractTxError::AbsurdFeeRate { fee_rate, .. } => fee_rate,
                _ => panic!(""),
            }),
            Err(FeeRate::from_tap_per_kwu(6250003)) // 6250000 is 25k tap/vbyte
        );

        // Lowering the input tapyruses by 1 lowers the tap/kwu by 3
        // Putting it exactly at 25k tap/vbyte
    ...
    fn serialize_then_deserialize_global() {
        let expected = Psbt {
            unsigned_tx: Transaction {
                ...
                output: vec![
                    TxOut {
                        value: Amount::from_tap(99_999_699),
                        script_pubkey: ScriptBuf::from_hex(
                    ...
                    TxOut {
                        value: Amount::from_tap(100_000_000),
                        script_pubkey: ScriptBuf::from_hex(
                            "a9143545e6e33b832c47050f24d3eeb93c9c03948bc787",
                        )
                        .unwrap(),
    ...
    fn test_serde_psbt() {
        ...
        let tx = Transaction {
            ...
            output: vec![TxOut {
                value: Amount::from_tap(190_303_501_938),
                script_pubkey: ScriptBuf::from_hex(
                    "a914339725ba21efd62ac753a9bcd067d6c7a6a39d0587",
                )
                .unwrap(),
    ...
    mod bip_vectors {
                ...
                inputs: vec![
                    Input {
                        non_witness_utxo: Some(Transaction {
                            ...
                            output: vec![
                                TxOut {
                                    value: Amount::from_tap(200_000_000),
        ...
        fn valid_vector_3() {
            ...
            assert_eq!(tx_input.previous_output.txid, psbt_non_witness_utxo.malfix_txid());
    ...
    #[test]
    fn test_fee() {
        let output_0_val = Amount::from_tap(99_999_699);
        let output_1_val = Amount::from_tap(100_000_000);
        let prev_output_val = Amount::from_tap(200_000_000);
