...
impl<R: BorrowMut<Transaction>> SighashCache<R> {
    ...
    /// ```
    /// use tapyrus::{absolute, transaction, Amount, Transaction, Script};
    /// use tapyrus::sighash::{EcdsaSighashType, SighashCache};
    ///
...
impl<E> EncodeSigningDataResult<E> {
    ...
    /// # use tapyrus::hashes::{Hash, hex::FromHex};
    /// # use tapyrus::sighash::{LegacySighash, SighashCache};
    /// # use tapyrus::Transaction;
    /// # let mut writer = LegacySighash::engine();
    /// # let input_index = 0;
    /// # let script_pubkey = tapyrus::ScriptBuf::new();
