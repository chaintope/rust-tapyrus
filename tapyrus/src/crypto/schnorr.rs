...
use crate::crypto::prime::jacobi;
use crate::crypto::rfc6979::nonce_rfc6979;
...
/// "SCHNORR + SHA256"
pub const ALGO16: [u8; 16] = [83, 67, 72, 78, 79, 82, 82, 32, 43, 32, 83, 72, 65, 50, 53, 54];
...
impl Signature {
    ...
    pub fn sign(privkey: &PrivateKey, message: &[u8; 32]) -> Result<Self, Error> {
        Self::sign_inner(&privkey.inner, message)
    }
    ...
    fn sign_inner(sk: &SecretKey, message: &[u8; 32]) -> Result<Self, Error> {
        ...
        let sigma = {
            let mut result = e;
            result = result.mul_tweak(&Scalar::from(*sk))?;
            result = result.add_tweak(&Scalar::from(k))?;
            result
        };
    ...
    pub fn verify(&self, message: &[u8; 32], pk: &PublicKey) -> Result<(), Error> {
        self.verify_inner(message, &pk.inner)
    }
    ...
    fn verify_inner(&self, message: &[u8; 32], pk: &secp256k1::PublicKey) -> Result<(), Error> {
        ...
        let r = {
            ...
            let minus_ep = {
                let mut result = *pk;
                result = result.mul_tweak(&ctx, &Scalar::from(e))?;
                result
            };

            let sg = {
                let mut result = PublicKey::generator().inner;
                result = result.mul_tweak(&ctx, &Scalar::from(s))?;
                result
            };
    ...
    fn generate_k(sk: &SecretKey, message: &[u8; 32]) -> SecretKey {
        ...
        loop {
            let nonce = nonce_rfc6979(message, sk, &ALGO16, None, count);
            count += 1;
...
impl Default for Signature {
    fn default() -> Self {
        Signature { sigma: [0u8; SECP256K1_SCALAR_SIZE], r_x: [0u8; SECP256K1_SCALAR_SIZE] }
    }
}
...
impl From<secp256k1::Error> for Error {
    fn from(e: secp256k1::Error) -> Error { Error::Secp256k1Error(e) }
}
...
mod tests {
    ...
    use crate::crypto::test_helpers::{
        decode_message, decode_pk, decode_signature, decode_sk, pk_from,
    };
    ...
    fn test_p2p_sign_and_verify() {
        for n in 0..16 {
            let msg = {
                ...
                let hash = crate::hashes::sha256::Hash::hash(m.as_bytes());
                hash.to_byte_array()
            };

            let key = PrivateKey::from_wif("5HxWvvfubhXpYYpS3tJkw6fq9jE9j18THftkZjHHfmFiWtmAbrj")
                .unwrap();
    ...
    fn test_signing_and_verification() {
        let default_message =
            decode_message("243F6A8885A308D313198A2E03707344A4093822299F31D0082EFA98EC4E6C89");
        let default_pk =
            decode_pk("02DFF1D77F2A671C5F36183726DB2341BE58FEAE1DA2DECED843240F7B502BA659");
        ...
        assert_eq!("06705D6B7FD5A7A34EA47B6A8D0CE8372A83D2129A65458E2BEF6F45892E7D5DBB13B346C6937CB76D25EFB18979B6523C72B56DD13B8F9D2F180893D20ECF45",
                   serialize(&sign).to_upper_hex_string());
        ...
        let pk = pk_from(&sk);
        let message =
            decode_message("5E2D58D8B3BCDF1ABADEC7829054F90DDA9805AAB56C77333024B9D0A508B75C");
        let sign = Signature::sign_inner(&sk, &message).unwrap();
        ...
        let pk = pk_from(&sk);
        let message =
            decode_message("FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF");
        let sign = Signature::sign_inner(&sk, &message).unwrap();
        ...
        let message =
            decode_message("4DF3C3F68FCC83B27E9D42C90431A72499F17875C81A599B566C9889B9696703");
        ...
        let pk =
            &Vec::from_hex("02EEFDEA4CDB677750A420FEE807EACF21EB9898AE79B9768766E4FAA04A2D4A34")
                .unwrap();
        ...
        let pk =
            &Vec::from_hex("02FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEFFFFFC30")
                .unwrap();
        assert!(secp256k1::PublicKey::from_slice(&pk[..]).is_err());
    }
    ...
    fn test_decode() {
        ...
        let r_x = <[u8; 32]>::from_hex(
            "6ba8aee2e8cee077cb4a799c770e417fb750586ee5dd9f61db65f5158a596e77",
        )
        .unwrap();
        let sigma = <[u8; 32]>::from_hex(
            "aaa87e4fec16c70b102bbe99a6c4fe77be424a44a2f5cfdc5fe04d5b4bca799c",
        )
        .unwrap();
        ...
        assert_eq!(real_decode.sigma, sigma);
        assert_eq!(real_decode.r_x, r_x);
